...
int main(int argc, const char **argv) {
  ...
  if (c.findClass({"Cadena"})) {
    c.defaultConstructor(
        "Cadena", "Revisa el enunciado respecto a la construcción de objetos.");

    vector<string> params = {"unsigned long", "char"};
    c.explicitSpecifiedConstructor(
        "Cadena", params,
    ...
    vector<string> memoryFunctionNames = {"malloc", "calloc", "realloc",
                                          "free"};
    string memoryHeaderName = "stdlib";
    c.invocationsFromHeaders(
        memoryFunctionNames, memoryHeaderName, false,
        "Revisa la asignación y destrucción de memoria de variables.");

    c.takeException(
        {"out_of_range"},
        "Revisa las excepciones que se han de lanzar en el programa.");
    c.fileIncludedHeader(
        "stdexcept", {"cadena.cpp", "cadena.hpp"},
        "Comprueba haber incluido las cabeceras adecuadas en tus ficheros.");

    vector<string> standardFunctionNames = {"strlen", "strcat", "memset",
                                            "strcpy"};
    ...
    c.inlineMethod(methodNames, parametersMethods, "Cadena", {"?"},
                   "Sugerencia: incluir marca 'inline' a aquellos métodos con "
                   "pocas instrucciones, como 'length()'.");
    ...
    c.moveConstructor("Cadena",
                      "Revisa el enunciado respecto a los constructores.");

    c.notFriendMember("Cadena",
    ...
    c.methodWithReferencedMethod(
        {"rbegin", "crbegin", "rend", "crend"}, parametros, "Cadena",
        {"?", "?", "?", "?"}, {"end", "end", "begin", "begin"}, parametros,
        "Cadena", {"?", "?", "?", "?"},
