...
Pedido::Pedido(Usuario_Pedido &up, Pedido_Articulo &pa, Usuario &user,
               const Tarjeta &tar, const Fecha &f)
    : num_(cant_ + 1), tarjeta_(&tar), fecha_(f), total_(0) {
  if (user.n_articulos() == 0) throw Vacio(user);
  if (tar.titular() != &user) throw Impostor(user);
  if (!tar.activa()) throw Tarjeta::Desactivada();
  if (f > tar.caducidad()) throw Tarjeta::Caducada(tar.caducidad());
...
int Pedido::n_total_pedidos() noexcept { return cant_; }
